import { computeSpamScore, type UserProfile, type PostingStats } from './index';

const NORMAL_USER: UserProfile = {
...
const NORMAL_POSTING: PostingStats = {
  posts_last_hour: 0,
  posts_last_24h: 2,
};

const RAPID_POSTING: PostingStats = {
  posts_last_hour: 5,
  posts_last_24h: 15,
};
...
export const TEST_EXAMPLES = [
  {
    ...
    reason: '1 phone, experienced user with good reputation',
  },
  {
  ...
  },
  {
    ...
    expectedScore: '0-20',
    expectedStatus: 'published',
    reason: '1 link, good reputation cancels penalty',
  },
  {
    ...
    reason: '3 phones, 2 links, 11 hashtags, rapid posting, new account',
  },
  {
    ...
    expectedScore: '40-60',
    expectedStatus: 'published',
    reason: 'Excessive caps, 1 phone, 1 link, new account',
  },
  {
    ...
    expectedScore: '25-35',
    expectedStatus: 'published',
    reason: 'Duplicate post, but good user reduces penalty',
  },
];
...
export function runTests() {
  ...
  TEST_EXAMPLES.forEach((example, index) => {
    ...
    console.log(`Status: ${result.status === example.expectedStatus ? '✅ PASS' : '⚠️  CHECK'}`);
  });
