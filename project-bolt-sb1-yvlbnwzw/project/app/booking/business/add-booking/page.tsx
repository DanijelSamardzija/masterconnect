...
export default function OwnerAddBookingPage() {
  ...
  }, [locationId, serviceId, staffId, week, fetchSlots]);
  ...
  useEffect(() => {
    ...
    for (const d of weekDays) {
      ...
      const count = slots.filter(s => s.available && tzDateKey(s.slot_start, tz) === key).length;
      if (count > 0) { setSelectedDay(key); return; }
    }
    setSelectedDay('');
  }, [slots, slotsLoading, week, timezone]);

  async function handleSubmit() {
    if (!serviceId || !staffId || !slotStart || !guestName.trim()) return;
    setSubmitting(true);
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {staff.map((s) => (
                      <button
                        key={s.id}
                        type="button"
                        onClick={() => { setStaffId(s.id); setSlotStart(''); setSelectedDay(''); }}
              ...
              <button
                ...
                disabled={submitting || !serviceId || !staffId || !slotStart || !guestName.trim()}
              ...
              >
                {submitting ? '...' : t('ownerBookings.add.save')}
              </button>
            </div>
          )}
        </div>
