...
import {
  ...
  Check, Calendar, X, MapPin, Share2, Phone
} from 'lucide-react';
    ...
    async function loadMeta() {
      ...
      try {
        const [bizRes, svcRes, locRes, rulesRes] = await Promise.all([
          (supabase as any).from('booking_profiles').select('id, name, contact_channels').eq('id', businessId).eq('is_active', true).maybeSingle(),
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        {/* Location + contact strip — shown when selected location has address or phone */}
        {selectedLocation && (selectedLocation.address || selectedLocation.city || selectedLocation.phone || (business?.contact_channels?.phone)) && (
          <div className="mb-5 flex flex-col gap-1.5">
            {(selectedLocation.address || selectedLocation.city) && (() => {
              const locLabel = [selectedLocation.address, selectedLocation.city, selectedLocation.country].filter(Boolean).join(', ');
              const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(locLabel)}`;
              return (
                <a
                  href={mapsUrl}
                  target="_blank"
            ...
            {(() => {
              const phone = selectedLocation.phone || business?.contact_channels?.phone || null;
