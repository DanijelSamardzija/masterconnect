...
export default function SlotPickerPage() {
  ...
  // Load business + service + locations
  useEffect(() => {
    ...
    async function loadMeta() {
      ...
      const [bizRes, svcRes, locRes] = await Promise.all([
        ...
        supabase.from('service_catalog').select('id, name, description, duration_minutes, capacity, price, price_type').eq('id', serviceId).eq('business_id', businessId).eq('is_active', true).maybeSingle(),
  ...
  // Group slots by calendar day (in the location's timezone)
  ...
  async function handleBook() {
    ...
    if (!result?.ok) {
      toast.error(t(bookingErrorKey(result?.error ?? '')));
      return;
    }
    setBooked(true);
    setDialogOpen(false);
  ...
  if (!business || !service) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4 p-6">
        ...
        <button onClick={() => router.push(`/book/${businessId}`)} className="text-primary underline text-sm">
                  ...
                  {daySlots
                    ...
                    .map((s) => (
                      <button
                        ...
                        className="text-xs py-1.5 px-0.5 rounded-md bg-primary/10 hover:bg-primary/20 text-primary font-medium transition-colors text-center w-full"
                      >
