...
function JobDetailsContent() {
  ...
  const handleContactCustomer = async () => {
    ...
    if (job.customer_id === user.id) {
      toast.error('Ne možete kontaktirati svoj posao');
      return;
    }
  ...
  const fetchJob = async () => {
    const { data, error: fetchError } = await supabase
      ...
      .select(`
        *,
        customer:profiles!jobs_customer_id_fkey(name, email)
      `)
      .eq('id', jobId)
      .maybeSingle();
    ...
    if (data.status === 'completed') {
      ...
      const { data: threadData } = await supabase
        ...
        .select(`
          ...
          user1_id,
          user2_id,
          user1:profiles!threads_user1_id_fkey(name),
          user2:profiles!threads_user2_id_fkey(name)
        `)
        .eq('job_id', jobId)
      ...
      if (threadData) {
        ...
        setJobThread({
          id: threadData.id,
          pro_id: otherUserId,
          pro_name: (otherUser as any)?.name || '',
        });
      }
    }
