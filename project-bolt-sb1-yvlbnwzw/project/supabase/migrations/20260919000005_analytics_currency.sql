-- Analytics: return dominant currency from completed bookings so the UI
-- can format money in the correct currency instead of always using EUR.
...
BEGIN
  ...
  -- Dominant currency among completed bookings in the filtered set
  SELECT COALESCE(
    (SELECT MODE() WITHIN GROUP (ORDER BY COALESCE(sc2.currency, 'EUR'))
     FROM   bookings b2
     LEFT   JOIN service_catalog sc2 ON sc2.id = b2.service_id
     WHERE  b2.business_id = v_biz_id
       AND  b2.status      = 'completed'
       AND  (p_location_id IS NULL OR b2.location_id      = p_location_id)
       AND  (v_from IS NULL        OR b2.starts_at        >= v_from)
       AND  (v_to   IS NULL        OR b2.starts_at        <  v_to)
       AND  (p_staff_id IS NULL    OR b2.staff_member_id  = p_staff_id)
       AND  (p_service_id IS NULL  OR b2.service_id       = p_service_id)
    ), 'EUR'
  ) INTO v_currency;
