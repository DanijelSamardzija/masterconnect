...
-- ── 8. Fix owner_create_booking for multi-business owners ─────────────────────
-- Previous version used LIMIT 1 to find the owner's business, which picked the
-- wrong business when the owner manages multiple locations.
-- Fix: derive business_id from the selected staff member, then verify ownership.
CREATE OR REPLACE FUNCTION public.owner_create_booking(
  p_service_id       uuid,
  p_staff_member_id  uuid,
  p_starts_at        timestamptz,
