-- Migration 039: Add primary_country to list_trade_businesses
-- Adds country to the primary-location lateral join so listing cards can show city + country.
...
BEGIN
  ...
  FROM (
    ...
    -- Primary location (with city, country, address, timezone for closure conversion)
    LEFT JOIN LATERAL (
      SELECT id, city, country, address, COALESCE(timezone, 'Europe/Belgrade') AS timezone
