-- Extend staff_get_my_shifts to also include absence days from time_blocks.
-- Previously the RPC only read staff_shifts, so vacation/sick-leave periods
-- set by the owner in "Odsustva" were invisible to the staff member in
-- "Moj raspored".
--
-- Strategy: expand each time_block range into individual dates, then UNION
-- with staff_shifts.  Dates already covered by a staff_shift row are kept
-- as-is (staff_shift takes precedence); absence dates fill the rest.
...
BEGIN
  ...
  RETURN COALESCE(
    (
      ...
      -- Expand each time_block into one row per calendar day (in business tz)
      absence_days AS (
        SELECT
        ...
        WHERE tb.staff_member_id = v_sm_id
          ...
          -- skip if a staff_shift already exists for that date
          AND NOT EXISTS (
            SELECT 1
            FROM staff_shifts ss2
            WHERE ss2.staff_member_id = v_sm_id
              AND ss2.shift_date      = d::DATE
          )
      ),
      combined AS (
        SELECT * FROM shifts
        UNION ALL
        SELECT * FROM absence_days
      )
      SELECT jsonb_agg(
        jsonb_build_object(
