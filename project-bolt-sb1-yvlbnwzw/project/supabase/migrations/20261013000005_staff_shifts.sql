-- ============================================================
-- Staff Shifts — per-date schedule overrides
--
-- 1.  staff_shifts table  (staff_member_id + shift_date unique)
-- 2.  owner_get_week_shifts(p_week_start)
-- 3.  owner_set_shift(...)
-- 4.  owner_delete_shift(staff_member_id, date)
-- 5.  owner_copy_week_shifts(from_week_start, to_week_start)
-- 6.  staff_get_my_shifts(from_date, to_date)
-- 7.  staff_set_my_shift(...)           [requires can_set_hours]
-- 8.  staff_delete_my_shift(date)       [requires can_set_hours]
-- 9.  Update get_available_slots — check staff_shifts FIRST
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.staff_shifts (
  ...
  CONSTRAINT staff_shifts_staff_date_uniq UNIQUE (staff_member_id, shift_date)
);

CREATE INDEX IF NOT EXISTS idx_staff_shifts_biz_date
  ON public.staff_shifts (business_id, shift_date);

CREATE INDEX IF NOT EXISTS idx_staff_shifts_sm_date
  ON public.staff_shifts (staff_member_id, shift_date);
...
GRANT EXECUTE ON FUNCTION public.owner_delete_shift(UUID, DATE) TO authenticated;


-- ── 5. owner_copy_week_shifts ────────────────────────────────────────────────
...
BEGIN
  ...
  -- Copy from source week, shifting dates by the week offset
  INSERT INTO staff_shifts (
    business_id, staff_member_id, location_id, shift_date,
    start_time, end_time, is_off, notes, created_by
  )
...
-- ── 8. staff_delete_my_shift ─────────────────────────────────────────────────

CREATE OR REPLACE FUNCTION public.staff_delete_my_shift(
  p_shift_date DATE
)
RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT sm.id, sm.permissions INTO v_sm_id, v_perms
  FROM staff_members sm
  WHERE sm.user_id = v_uid AND sm.is_active = true
  LIMIT 1;
  ...
  DELETE FROM staff_shifts
  WHERE staff_member_id = v_sm_id AND shift_date = p_shift_date;

  RETURN jsonb_build_object('ok', true);
END;
$$;

GRANT EXECUTE ON FUNCTION public.staff_delete_my_shift(DATE) TO authenticated;


-- ── 9. Update get_available_slots — staff_shifts override ────────────────────
-- Adds: v_shift_start, v_shift_end, v_shift_is_off, v_has_shift to DECLARE.
-- Before the opening_hours lookup: query staff_shifts for the exact date.
-- If is_off → CONTINUE (skip day). If working → use shift times, skip opening_hours.
...
BEGIN
  ...
  LOOP
    ...
    IF p_staff_member_id IS NOT NULL THEN
      SELECT ss.start_time, ss.end_time, ss.is_off
      INTO   v_shift_start, v_shift_end, v_shift_is_off
      FROM   staff_shifts ss
      ...
      IF FOUND THEN
        ...
        END IF;
        -- use shift times instead of opening_hours
