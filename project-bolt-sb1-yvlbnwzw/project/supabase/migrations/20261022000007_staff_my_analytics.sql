-- ==========================================================================
-- get_my_analytics — personal analytics for the calling staff member
--
-- Scoped entirely to auth.uid() via staff_members JOIN.
-- Workers see ONLY their own completed bookings — no cross-staff leakage.
-- Owners who call this also get their personal booking history (if any).
-- ==========================================================================
...
BEGIN
  ...
  -- Timezone from primary location of first active business
  ...
  -- Summary: completed count + revenue
  ...
  -- Per-service breakdown (grouped by service_name_snapshot for historical accuracy)
  SELECT COALESCE(jsonb_agg(row_data ORDER BY (row_data->>'completed')::int DESC), '[]'::jsonb)
  INTO   v_by_svc
  FROM (
    SELECT jsonb_build_object(
      'service_id',   COALESCE(b.service_id::TEXT, ''),
      'service_name', COALESCE(b.service_name_snapshot, sc.name, '—'),
      'completed',    COUNT(*),
    ...
    WHERE  b.status = 'completed'
      ...
      AND  (v_from IS NULL        OR b.starts_at  >= v_from)
      AND  (v_to   IS NULL        OR b.starts_at  <  v_to)
    GROUP  BY b.service_id, COALESCE(b.service_name_snapshot, sc.name)
  ) sub;

  -- Active locations of every business this person is a member of
