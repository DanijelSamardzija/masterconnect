-- Add anonymous flag to credit_transactions
ALTER TABLE credit_transactions ADD COLUMN IF NOT EXISTS anonymous boolean NOT NULL DEFAULT false;

-- Update send_credits to support anonymous flag + fix is_pro -> is_premium check
...
BEGIN
  ...
  IF p_anonymous THEN
    v_notif_title := 'Anonimni korisnik vam je poslao ' || p_amount || ' kredita';
  ELSE
    v_notif_title := COALESCE(v_sender_name, 'Neko') || ' vam je poslao ' || p_amount || ' kredita';
  END IF;
